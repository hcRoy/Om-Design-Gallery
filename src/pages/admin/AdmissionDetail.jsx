...
export default function AdmissionDetail() {
  ...
  const [printLanguage, setPrintLanguage] = useState('gu')
  ...
  const load = useCallback(async () => {
    ...
    setPrintLanguage(row?.preferred_language === 'en' ? 'en' : 'gu')
  ...
  const handlePrint = useReactToPrint({
    contentRef: printRef,
    documentTitle: `admission-${admission?.form_number ?? id}`,
  })
  ...
  return (
    <>
      ...
      <PageHeader
        ...
        action={
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button type="button" className="btn-primary !text-xs !py-2" onClick={() => setPrintOpen(true)}>
              Preview &amp; Print
            </button>
          </div>
        }
      ...
      <Modal open={printOpen} onClose={() => setPrintOpen(false)} title="Print preview" size="xl">
          ...
          <div className="inline-flex rounded-xl border border-ink/10 bg-sand p-1">
            <button
              ...
              className={`px-3 py-1.5 text-xs font-bold rounded-lg ${
                printLanguage === 'en' ? 'bg-maroon text-ivory' : 'text-ink-soft'
              }`}
            >
              English
            </button>
            <button
              ...
              className={`px-3 py-1.5 text-xs font-bold rounded-lg ${
                printLanguage === 'gu' ? 'bg-maroon text-ivory' : 'text-ink-soft'
              }`}
            >
              ગુજરાતી
            </button>
          </div>
          <button type="button" className="btn-primary !text-xs !py-2" onClick={() => handlePrint()}>
            Print / Save PDF
          </button>
        </div>
